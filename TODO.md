...
- [~] `main.cpp:13865` — Pressing G (or scrolling deep) on a huge streaming file loads the entire file into RAM with no eviction — DelimitedSource done on `perf/streaming-batch-eviction` (CSV/TSV/BED/VCF/GFF/SAM/PAF/mpileup now keep a bounded trailing window of decoded batches, default 64, env VV_STREAM_BATCH_CAP; per-batch first_row/num_rows metadata is retained forever so total_rows()/chunk_meta()/num_chunks() stay exact; read_chunk() of an evicted batch returns CapacityError → the TUI renders those rows blank. Sequential single-pass consumers — export/--tsv/--csv/--json/--md, --unique, table view, and the TUI forward-browse + G — stay correct AND become bounded. Full-pass consumers that drain-then-reread (find_next search, compute_col_stats, filter_rows, rebuild_display_order sort/filter) call set_retain_all(true) first, so they keep their current behaviour (correct; same RAM as before — no regression). Forced-eviction export/unique-completeness regression tests added. The forward-only nature means search/sort/back-scroll over a fully-browsed huge stream is limited to retained data — the accepted tradeoff of the RAM-ring approach (no temp files / no re-open).
  - Follow-up: apply the identical bounded-window + set_retain_all pattern to the other six forward-only streaming sources (BamSource, BamPileupSource, BcfSource, BigSource, FastxSource, SqliteSource) — mechanical (same retain_/chunk_meta/read_chunk/set_retain_all shape). IpcSource/OrcSource are random-access (re-read on miss) so they can bound with zero correctness cost as a separate, strictly-better variant.
...
- [ ] `main.cpp` — `--describe` / `print_describe` reports stats over only the first ~10 rows of a large streaming file (count shows 10 for a 5M-row TSV); pre-existing, unrelated to the eviction work. Likely a head_rows/rows_left default leaking into the describe scan. Investigate and make `--describe` scan all rows (or document the sample size).
