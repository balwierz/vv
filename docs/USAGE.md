...
| AnnData / HDF5    | `.h5ad`, `.h5`, `.hdf5`, `.loom` via libhdf5. AnnData is detected by the root `encoding-type` attribute (or the modern heuristic `/obs` + `/var` + `X` all present). Tab 0 = summary (shape, X encoding, layer count); subsequent tabs are `obs`, `var`, `X` (preview), and each entry under `obsm` / `varm` / `layers` — sortable / filterable through the regular `TableTUI` machinery. Sparse `X`, `layers`, `obsm` and `varm` entries (CSR or CSC group: `encoding-type="csr_matrix"` / `"csc_matrix"`, child `data` / `indices` / `indptr` datasets) get a densified preview of the first 1000 rows and 200 columns with cell / gene labels; the footer reports shape + format. `uns` becomes a key/value tab (scalars, strings, short arrays; nested dicts flattened to dotted keys). AnnData categoricals (`encoding-type="categorical"` group with `codes` + `categories`) are decoded back to string columns. Generic HDF5 (`.h5`, `.hdf5`, `.loom`) gets a hierarchy table (`path` / `kind` / `shape` / `dtype` / `n_attrs`) plus a tab for every 1-D and 2-D dataset (up to 32 datasets so a chunky file doesn't explode the tab bar; each 2-D dataset is previewed at 1000 rows × 200 columns). Datasets compressed with gzip or LZF (h5py `compression="lzf"`, HDF5 filter 32000, decoded by vv itself) are read; a dataset that cannot be decoded is reported with the dataset and missing filter named — a scripted export of that tab exits 1 — rather than shown as zeros. Cell Ranger HDF5 (`filtered_feature_bc_matrix.h5`: v3's `/matrix` group or v2's per-genome groups) opens as `summary` (shape, stored entries, counts per `feature_type`, chemistry), `matrix (preview)` — stored features × barcodes, shown cells × features, labelled by barcode and feature name (a repeated name gets its id appended) — `features` (`id`, `name`, `feature_type`, `genome`, …) and `barcodes`. Loom files (loompy / velocyto / SCope: `/matrix` + `/row_attrs` + `/col_attrs`) open as `summary` (spec version, shape, layers, attributes, graphs), `matrix (preview)` and one `layers[…]` tab per layer — stored genes × cells, shown cells × genes, labelled by the cell and gene ID attributes (`CellID` / `Gene`, or `obs_names` / `var_names` and other writers' names; a repeated gene name gets its accession appended) — plus `cells` (`col_attrs`) and `genes` (`row_attrs`) tables. |
