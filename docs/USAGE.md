...
| AnnData / HDF5    | `.h5ad`, `.h5`, `.hdf5`, `.loom` via libhdf5. AnnData is detected by the root `encoding-type` attribute (or the modern heuristic `/obs` + `/var` + `X` all present). Tab 0 = summary (shape, X encoding, layer count); subsequent tabs are `obs`, `var`, `X` (preview), and each entry under `obsm` / `varm` / `layers` — sortable / filterable through the regular `TableTUI` machinery. Sparse `X` (CSR group with `encoding-type="csr_matrix"`, child `data` / `indices` / `indptr` datasets) gets a densified first-1000-row preview; the footer reports shape + format. AnnData categoricals (`encoding-type="categorical"` group with `codes` + `categories`) are decoded back to string columns. Generic HDF5 (`.h5`, `.hdf5`, `.loom`) gets a hierarchy table (`path` / `kind` / `shape` / `dtype` / `n_attrs`) plus a tab for every 1-D and 2-D dataset (capped at 32 columns / 32 datasets so a chunky file doesn't explode the tab bar). CSC sparse and `uns` (unstructured) decoding are deferred. |
