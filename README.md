...
manifest. Replace `1.23.0` below with the
[latest release](https://github.com/balwierz/vv/releases/latest) if newer.

### Debian / Ubuntu

```sh
curl -LO https://github.com/balwierz/vv/releases/download/v1.23.0/vv_1.23.0-1_amd64.deb
sudo apt install ./vv_1.23.0-1_amd64.deb
```
...
```sh
curl -LO https://github.com/balwierz/vv/releases/download/v1.23.0/vv-gui_1.23.0-1+ubuntu24.04_amd64.deb
sudo apt install ./vv-gui_1.23.0-1+ubuntu24.04_amd64.deb
```
...
```sh
base=https://github.com/balwierz/vv/releases/download/v1.23.0
curl -LO $base/vv-1.23.0-linux-x86_64.tar.gz
curl -LO $base/SHA256SUMS
sha256sum --check --ignore-missing SHA256SUMS

tar -xzf vv-1.23.0-linux-x86_64.tar.gz
sudo install vv-1.23.0-linux-x86_64/vv /usr/local/bin/
```
...
| AnnData / HDF5    | `.h5ad`, `.h5`, `.hdf5`, `.loom` (single-cell + generic). AnnData files surface as a summary tab (shapes, plus a profile per matrix — dtype, stored fraction and a sampled max / whole-number check, so raw counts and log-normalised values can be told apart) plus obs / var / X-preview / obsm / varm / layers / uns tabs, and `raw.X` / `raw.var` when the file keeps `.raw` (raw.X labelled by raw/var's genes); `obsp` / `varp` graphs (e.g. neighbour connectivities) open as streamed edge lists `i, j, obs_i, obs_j, weight`, never densified; sparse (CSR / CSC) matrices get a dense first-N-row preview. obs / var export in full; exporting a capped matrix preview (`--tab X --tsv`) exits 1 with the real shape rather than writing part of it. gzip- and LZF-compressed datasets are read. Generic HDF5 opens with a hierarchy table and one tab per 1D / 2D dataset. Cell Ranger `filtered_feature_bc_matrix.h5` (v3 and v2) opens as summary / labelled cells × features matrix / features / barcodes tabs. Loom files open as summary / labelled cells × genes matrix and layers / cells / genes tabs. |
