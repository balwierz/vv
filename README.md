...
| AnnData / HDF5    | `.h5ad`, `.h5`, `.hdf5`, `.loom` (single-cell + generic). AnnData files surface as a summary tab plus obs / var / X-preview / obsm / varm / layers / uns tabs; sparse (CSR / CSC) matrices get a dense first-N-row preview. gzip- and LZF-compressed datasets are read. Generic HDF5 opens with a hierarchy table and one tab per 1D / 2D dataset. |
